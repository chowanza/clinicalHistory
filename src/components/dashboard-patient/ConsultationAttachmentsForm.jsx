...
const ConsultationAttachmentsForm = ({ patientId, consultationId, attachments: initialAttachments = [], onUpload }) => {
  const [selectedFiles, setSelectedFiles] = useState([]);
  const [uploading, setUploading] = useState(false);
  const [attachments, setAttachments] = useState(initialAttachments);
  const fileInputRef = useRef();

  // Cargar archivos actuales de la consulta
  const fetchAttachments = async () => {
    const res = await fetch(`/api/tasks/${patientId}/consultations/${consultationId}`);
    if (res.ok) {
  ...
  // Cargar al montar
  useState(() => { fetchAttachments(); }, [patientId, consultationId]);
  ...
  const handleUpload = async (e) => {
    ...
    const res = await fetch(`/api/tasks/${patientId}/consultations/${consultationId}/attachments`, {
      method: 'POST',
      body: formData,
      credentials: 'include',
    });
  ...
  const handleDelete = async (file) => {
    ...
    await fetch(`/api/tasks/${patientId}/consultations/${consultationId}/attachments/${file.filename}`, {
      method: 'DELETE',
      credentials: 'include',
    });
    fetchAttachments();
